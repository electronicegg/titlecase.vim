...
function! s:DoAction(algorithm, type)
    ...
    " yank the relevant text, and also set the visual selection
    " (which will be reused if the text needs to be replaced)
    if a:type =~ '^\d\+$'
        " if type is a number, then select that many lines
        silent exe 'normal! V'.a:type.'$y'

    elseif a:type =~ '^.$'
        " if type is 'v', 'V', or '<C-V>' (i.e. 0x16) then reselect the visual
        " region
    ...
    " call the user-defined function, passing it the contents of the unnamed
    " register
    let repl = s:{a:algorithm}(@@)

    " if the function returned a value, then replace the text
    if type(repl) == 1
        " put the replacement text into the unnamed register, and also set it
        " to be a characterwise, linewise, or blockwise selection, based upon
        " the selection type of the yank we did above
        call setreg('@', repl, getregtype('@'))

        " select the visual region and paste
        normal! gvp
    endif
